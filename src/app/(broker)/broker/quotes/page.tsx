      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        {(
          ...
          >
            ...
            {val === "all"
              ? `(${quotes.length})`
              : `(${quotes.filter((q) => q.status === val).length})`}
          </button>
        ))}
      </div>
            ...
            <tbody>
              {filtered.map((q, i) => (
                ...
                >
                  ...
                  <td className="px-4 py-3.5 font-semibold text-[#1A2A3A]">
                    {q.client_name}
                  </td>
                  <td className="px-4 py-3.5 text-slate-500">{q.city}</td>
                  <td className="px-4 py-3.5 text-slate-500 text-xs">
                    {SYSTEM_LABELS[q.system_type ?? "onGrid"] ?? q.system_type ?? "OnGrid"}
                  </td>
                  <td className="px-4 py-3.5">{q.kwp} kWp</td>
                  <td className="px-4 py-3.5 font-semibold">{fmt(q.project_value_cop)}</td>
                  <td className="px-4 py-3.5 text-green-600 font-bold">
                    {fmt(q.commission_month1)}
                  </td>
                  <td className="px-4 py-3.5">
                    <span
                      className={`text-xs font-semibold px-2.5 py-1 rounded-full ${STATUS_COLORS[q.status]}`}
                    >
                  ...
                  <td className="px-4 py-3.5">
                    <Link
                      ...
                      className="bg-[#1A2A3A] text-white text-xs px-3 py-1.5 rounded-lg hover:bg-[#243447] transition font-semibold"
                    >
                      Ver propuesta
                    </Link>
                  </td>
        ...
        <div className="mt-4 bg-white rounded-xl p-4 flex gap-8 shadow-sm flex-wrap">
          {(
            [
              [
                "Total proyectos",
                fmt(filtered.reduce((s, q) => s + q.project_value_cop, 0)),
              ],
              [
                "Comisiones potenciales (mes 1)",
                fmt(filtered.reduce((s, q) => s + q.commission_month1, 0)),
              ],
              [
                "Promedio por proyecto",
                fmt(
                  filtered.reduce((s, q) => s + q.project_value_cop, 0) /
                    filtered.length
                ),
              ],
            ] as [string, string][]
